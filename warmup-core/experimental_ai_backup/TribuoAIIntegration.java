...
public class TribuoAIIntegration {
    ...
    public ClassificationMetrics evaluateClassificationModel(
        ...
        try {
            ...
            // Get confusion matrix using Tribuo 4.3.1 API
            Map<Label, Map<Label, Double>> confusionMatrix = new HashMap<>();
            ImmutableOutputInfo<Label> labelInfo = evaluation.getPredictions().get(0).getModel().getOutputIDInfo();
            for (Label predicted : labelInfo.getDomain()) {
                Map<Label, Double> row = new HashMap<>();
                for (Label actual : labelInfo.getDomain()) {
    ...
    public Model<Regressor> trainRegressionModel(
        ...
        try {
            ...
            // Create Linear SGD trainer with correct Tribuo 4.3.1 API
            LinearSGDTrainer trainer = new LinearSGDTrainer(
                new SquaredLoss(),  // RegressionObjective
                new org.tribuo.math.optimisers.AdaGrad(learningRate, 0.1),
                epochs,
                Math.max(1, trainingData.size() / 4),  // minibatch size
                1L  // seed
            );
    ...
    public RegressionMetrics evaluateRegressionModel(
        ...
        try {
            ...
            // Extract metrics for all dimensions using Tribuo 4.3.1 API
            Map<String, DimensionMetrics> dimensionMetrics = new HashMap<>();
            
            // Get dimension names from the regressor
            Set<String> dimensionNames = evaluation.getRegressorID();
            for (String dimName : dimensionNames) {
                DimensionMetrics dimMetrics = new DimensionMetrics(
                    dimName,
                    evaluation.r2(dimName),
                    evaluation.rmse(dimName),
                    evaluation.mae(dimName),
                    evaluation.mse(dimName)  // Use mse instead of averageSquaredError
                );
            ...
            double avgR2 = 0.0, avgRMSE = 0.0, avgMAE = 0.0;
            int numDims = dimensionMetrics.size();
            if (numDims > 0) {
                for (DimensionMetrics dm : dimensionMetrics.values()) {
                    avgR2 += dm.r2;
                    avgRMSE += dm.rmse;
                    avgMAE += dm.mae;
                }
    ...
    /**
     * Model information for AIService compatibility
     */
    public static class ModelInfo {
        ...
        public ModelInfo(String modelName, String modelType, boolean isActive) {
            this.modelName = modelName;
            this.modelType = modelType;
        ...
        @Override
        public String toString() {
            return String.format("ModelInfo{name='%s', type='%s', active=%s}",
                modelName, modelType, isActive);
        }
    }
    
    /**
     * Evaluation result for AIService compatibility
     */
    public static class EvaluationResult {
        ...
        public EvaluationResult(String modelName, double accuracy, double precision, double recall) {
            this.modelName = modelName;
            this.accuracy = accuracy;
            this.precision = precision;
            this.recall = recall;
        ...
        @Override
        public String toString() {
            return String.format("EvaluationResult{model='%s', accuracy=%.4f, precision=%.4f, recall=%.4f}",
