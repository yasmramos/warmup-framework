...
import io.warmup.framework.hotreload.advanced.SimplifiedAdvancedHotReloadManager;
import io.warmup.framework.hotreload.advanced.HotReloadDashboard;
...
public class SimpleAdvancedHotReloadExample {
    ...
    private static void displayMetrics(SimpleAdvancedHotReloadSystem system) {
        ...
        // Métricas globales
        GlobalMetrics globalMetrics = system.getGlobalMetrics();
        ...
        // Estado de salud
        SystemHealthStatus health = system.getSystemHealthStatus();
        log.log(Level.INFO, "Salud del Sistema: {0}", health.getOverallStatus().getDisplayName());
        
        if (health.getOverallStatus() == SystemHealthStatus.HealthStatus.WARNING) {
            log.log(Level.INFO, "Advertencias: Performance degradation detected");
        } else if (health.getOverallStatus() == SystemHealthStatus.HealthStatus.CRITICAL) {
            log.log(Level.INFO, "Estado crítico detectado");
        }
        
        // Reporte completo
        log.log(Level.INFO, "\n=== REPORTE COMPLETO ===\n{0}", system.generateReport());
    }
    ...
    public static class SimpleAdvancedHotReloadSystem {
        ...
        public String generateReport() {
            return dashboard.generateReport();
        }
    }
}